...
use IngestionEngine\Connector\Helper\Import\Attribute as AttributeHelper;
...
class Attribute extends Import
{
     ...
     */
    const DEFAULT_ATTRIBUTE_SET_NAME = 'IngestionEngine';
    /**
    ...
    public function createTable()
    {
        ...
        $attributes = $this->ingestionengineClient->getAttributeApi()->listPerPage(1);
        /** @var array $attribute */
        $attribute = $attributes->getItems();
        if (empty($attribute)) {
            $this->setMessage(__('No results from IngestionEngine'));
            $this->stop(1);
    ...
    public function insertData()
    {
        ...
        $attributes = $this->ingestionengineClient->getAttributeApi()->all($paginationSize);
        /** @var [] $metricsSetting */
        ...
        foreach ($attributes as $index => $attribute) {
            ...
            if (ctype_digit(substr($attribute['code'], 0, 1))) {
                $this->setAdditionalMessage(__('The attribute %1 was not imported because it starts with a number. Update it in IngestionEngine and retry.', $attribute['code']));
                continue;
            }
            ...
            if ($attribute['type'] == 'pim_catalog_metric' && in_array(
                ...
                if ($attribute['scopable'] || $attribute['localizable']) {
                    $this->setAdditionalMessage(__('Attribute %1 is scopable or localizable please change configuration at Stores > Configuration > Catalog > IngestionEngine Connector > Products > Metrics management.', $attributeCode));
                    continue;
                }
    ...
    public function matchEntities()
    {
        ...
        $connection->query(
            $connection->insertFromSelect(
                $select,
                $this->entitiesHelper->getTable('ingestionengine_connector_entities'),
    ...
    public function matchFamily()
    {
        ...
        $familyAttributeRelationsTable = $this->entitiesHelper->getTable('ingestionengine_connector_family_attribute_relations');
    ...
    public function addAttributes()
    {
        ...
        while (($row = $query->fetch())) {
                ...
                if ($attributeFrontendInput !== $row['frontend_input'] && !in_array($row['code'], $this->excludedAttributes)) {
                    ...
                    if (is_array($mapping)) {
                        foreach ($mapping as $match) {
                            if (in_array($match['magento_attribute'], $this->excludedAttributes) && $row['code'] == $match['ingestionengine_attribute']) {
                                $skipAttribute = false;
                            }
                        }
                    }
            ...
            if ($skipAttribute === true) {
                /** @var string $message */
                $message = __('The attribute %1 was skipped because its type is not the same between IngestionEngine and Magento. Please delete it in Magento and try a new import', $row['code']);
            ...
            if ($row['_attribute_set_id'] && $row['group']) {
                ...
                foreach ($attributeSetIds as $attributeSetId) {
                    if (is_numeric($attributeSetId)) {
                        ...
                        /** @var bool $ingestionengineGroup */
                        $ingestionengineGroup = false;
                        /* Test if the default group was created instead */
                        if (!$groupId) {
                            $ingestionengineGroup = true;
                            $groupId     = $this->eavSetup->getSetup()->getTableRow(
                                'eav_attribute_group',
                                'attribute_group_name',
