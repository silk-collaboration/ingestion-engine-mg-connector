...
use IngestionEngine\Connector\Helper\Import\FamilyVariant as FamilyVariantHelper;
...
class FamilyVariant extends Import
{
    ...
    public function createTable()
    {
        ...
        $families = $this->ingestionengineClient->getFamilyApi()->all($paginationSize);
        /** @var bool $hasVariant */
        $hasVariant = false;
        /** @var array $family */
        foreach ($families as $family) {
            /** @var PageInterface $variantFamilies */
            $variantFamilies = $this->ingestionengineClient->getFamilyVariantApi()->listPerPage($family['code'], 1);
        ...
        if (!$hasVariant) {
            $this->setMessage(__('There is no family variant in IngestionEngine'));
            $this->stop();
        ...
        if (empty($variantFamily)) {
            $this->setMessage(__('No results retrieved from IngestionEngine'));
            $this->stop(1);
    ...
    public function updateProductModel()
    {
        ...
        $connection->query(
            $connection->updateFromSelect($query, ['p' => $this->entitiesHelper->getTable('ingestionengine_connector_product_model')])
        );
    }
    ...
    protected function insertFamilyVariantData($familyCode, $paginationSize)
    {
        ...
        $families = $this->ingestionengineClient->getFamilyVariantApi()->all($familyCode, $paginationSize);
        /**
         * @var int   $index
         * @var array $family
         */
