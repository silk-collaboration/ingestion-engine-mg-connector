...
use IngestionEngine\Connector\Block\Adminhtml\System\Config\Form\Field\Configurable as TypeField;
use IngestionEngine\Connector\Helper\Authenticator;
use IngestionEngine\Connector\Helper\IngestionEngineClient;
use IngestionEngine\Connector\Helper\Config as ConfigHelper;
use IngestionEngine\Connector\Helper\Import\Entities;
use IngestionEngine\Connector\Helper\Import\Product as ProductImportHelper;
use IngestionEngine\Connector\Helper\Output as OutputHelper;
use IngestionEngine\Connector\Helper\ProductFilters;
use IngestionEngine\Connector\Helper\Serializer as JsonSerializer;
use IngestionEngine\Connector\Helper\Store as StoreHelper;
use IngestionEngine\Connector\Job\Import as JobImport;
use IngestionEngine\Connector\Job\Option as JobOption;
...
class Product extends JobImport
{
    ...
    /**
     * IngestionEngine default association types, reformatted as column names
     *
     ...
     * @param IngestionEngineClient         $ingestionEngineClient
    ...
    public function __construct(
        ...
        IngestionEngineClient $ingestionengineclient,
        array $data = []
    ) {
        ...
        $this->ingestionengineclient         = $ingestionengineclient;
    ...
    {
        $products  = $this->ingestionengineclient->getProducts();
        ...
        $product = (array) reset($products);


        $this->entitiesHelper->createTmpTableFromApi($product, $this->getCode());

        $this->setMessage('createTable finished');
    }
    ...
    public function addRequiredData()
    {
        ...
        foreach ($matches as $match) {
            ...
            $pimAttribute = $match['ingestionengine_attribute'];
    ...
    public function createConfigurable()
    {
        ...
        $productModelTable = $this->entitiesHelper->getTable('ingestionengine_connector_product_model');
    ...
    public function checkEntities()
    {
        ...
        $ingestionengineConnectorTable = $this->entitiesHelper->getTable('ingestionengine_connector_entities');
    ...
    public function updateAttributeSetId()
    {
        ...
        $entitiesTable = $this->entitiesHelper->getTable('ingestionengine_connector_entities');
        /** @var Select $families */
    ...
    public function importFiles()
    {
        ...
        if (empty($attributesMapped)) {
            $this->setStatus(false);
            $this->setMessage(__('IngestionEngine Files Attributes is empty'));
        ...
        while (($row = $query->fetch())) {
            foreach ($attributeToImport as $attribute) {
                ...
                if (!$this->configHelper->mediaFileExists($filePath)) {
                    ...
                    $binary = $this->ingestionengineClient->getProductMediaFileApi()->download($row[$attribute]);
                    $this->configHelper->saveMediaFile($filePath, $binary);
                }

                // Change the IngestionEngine file path to Magento file path
    ...
    public function setWebsites()
    {
        ...
        if ($websiteAttribute != null) {
            ...
            if ($attribute->getAttributeId() != null) {
                ...
                $apiAttribute = $this->ingestionengineClient->getAttributeOptionApi()->all($websiteAttribute, 1);
                        ...
                        if ($associatedWebsites != null) {
                            ...
                            foreach ($associatedWebsites as $associatedWebsite) {
                                ...
                                if ($websiteSet === false) {
                                    ...
                                    $this->setAdditionalMessage(
                                        __(
                                            'Warning: The product with IngestionEngine id %1 has an option (%2) that does not correspond to a Magento website.',
                                            $row['identifier'],
                                            $optionLabel
                                        )
                                    );
                            ...
                            $this->setAdditionalMessage(
                                __(
                                    'Warning: The product with IngestionEngine id %1 has no associated website in the custom attribute.',
                                    $row['identifier']
                                )
                            );
                        }
    ...
    public function setCategories()
    {
        ...
        $select = $connection->select()
            ->from(['c' => $this->entitiesHelper->getTable('ingestionengine_connector_entities')], [])
            ->joinInner(
                ['p' => $tmpTable],
        ...
        $selectToDelete = $connection->select()
            ...
            ->joinInner(
                ['c' => $this->entitiesHelper->getTable('ingestionengine_connector_entities')],
    ...
    public function importMedia()
    {
        ...
        if (empty($gallery)) {
            $this->setStatus(false);
            $this->setMessage(__('IngestionEngine Images Attributes is empty'));
        ...
        while (($row = $query->fetch())) {
            ...
            foreach ($gallery as $image) {
                ...
                $media = $row[$image];
                /** @var string $name */
                $name = $this->entitiesHelper->formatMediaName(basename($media));
                /** @var string $filePath */
    ...
    {
        if (!$this->ingestionengineClient) {
            $this->ingestionengineClient = $this->getIngestionEngineClient();
            if (!$this->ingestionengineClient) {
                return [];
            }
        }
        ...
        $apiFamilies = $this->ingestionengineClient->getFamilyApi()->all($paginationSize);
        /** @var mixed[] $family */
