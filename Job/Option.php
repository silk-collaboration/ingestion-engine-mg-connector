...
use IngestionEngine\Connector\Helper\Import\Option as OptionHelper;
...
class Option extends Import
{
    ...
    public function createTable()
    {
        ...
        foreach ($attributes as $attribute) {
            if ($attribute['type'] == 'pim_catalog_multiselect' || $attribute['type'] == 'pim_catalog_simpleselect') {
                ...
                $options = $this->ingestionengineClient->getAttributeOptionApi()->listPerPage($attribute['code']);
    ...
    public function insertOptions()
    {
        ...
        $options = $connection->select()->from(['a' => $tmpTable], $columns)->joinInner(
                ['b' => $this->entitiesHelper->getTable('ingestionengine_connector_entities')],
    ...
    protected function processAttributeOption($attributeCode, $paginationSize)
    {
        ...
        $options = $this->ingestionengineClient->getAttributeOptionApi()->all($attributeCode, $paginationSize);
        /** @var int $index */
        $index = 0;
        /** @var array $option */
    ...
    public function getAllAttributes()
    {
        if (!$this->attributes) {
            ...
            /** @var string|int $paginationSize */
            $paginationSize = $this->configHelper->getPaginationSize();
            $this->attributes = $this->ingestionengineClient->getAttributeApi()->all($paginationSize);
        }
