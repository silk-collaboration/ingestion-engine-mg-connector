...
class Entities
{
     ...
     */
    const TABLE_NAME = 'ingestionengine_connector_entities';
    /**
     * @var array EXCLUDED_COLUMNS
     */
    const EXCLUDED_COLUMNS = ['_links'];
    /**
     * IngestionEngine Connector product import code
     *
    ...
    public function matchEntity($pimKey, $entityTable, $entityKey, $import, $prefix = null)
    {
        ...
        if ($count) {
            ...
            $maxCode = $connection->fetchOne(
                $connection->select()
                    ->from($ingestionengineConnectorTable, new Expr('MAX(`entity_id`)'))
                    ->where('import = ?', $import)
            );
    ...
    public function delete($import, $entityId)
    {
        ...
        /** @var string $pimTable */
        $pimTable = $this->getTable('ingestionengine_connector_entities');
    ...
    public function addJoinForContentStaging($select, $cols)
    {
        $select
            ...
            // We use "left join" to be able to create new product from IngestionEngine (they are not yet in catalog_product_entity)
        ...
        if (!$this->configHelper->isIngestionEngineMaster()) {
            $select->where(
